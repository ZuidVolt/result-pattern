...
@overload
def catch_iter[T_local, E_local: Exception, **P_local](
    exceptions: type[E_local] | tuple[type[E_local], ...],
    *,
    map_to: Any = None,
...
def catch_iter[T_local, E_local: Exception, **P_local](
    ...
    *,
    ...
    This decorator ensures that if an exception is raised during the iteration of
    the generator, it is caught and yielded as an `Err` variant.
    ...
    def decorator(f: Callable[P_local, Any]) -> Callable[P_local, Any]:
        @wraps(f)
        def wrapper(*args: P_local.args, **kwargs: P_local.kwargs) -> Any:
            original_gen = f(*args, **kwargs)
            if inspect.isasyncgenfunction(f):
                return SafeStreamAsync(_wrap_gen_async(original_gen, catch_tuple, exc_map, has_mapping=has_mapping))
            return SafeStream(_wrap_gen_sync(original_gen, catch_tuple, exc_map, has_mapping=has_mapping))
